...
## Derivatives of kinetic functions (requires SymPy)
@vars x

# Sodium current
dmNainf_sym = diff(mNainf(x),x)
dmNainf = lambdify(dmNainf_sym,[x])
dhNainf_sym = diff(hNainf(x),x)
dhNainf = lambdify(dhNainf_sym,[x])

# Potassium currents
dmKdinf_sym = diff(mKdinf(x),x)
dmKdinf = lambdify(dmKdinf_sym,[x])

dmAfinf_sym = diff(mAfinf(x),x)
dmAfinf = lambdify(dmAfinf_sym,[x])
dhAfinf_sym = diff(hAfinf(x),x)
dhAfinf = lambdify(dhAfinf_sym,[x])

dmAsinf_sym = diff(mAsinf(x),x)
dmAsinf = lambdify(dmAsinf_sym,[x])
dhAsinf_sym = diff(hAsinf(x),x)
dhAsinf = lambdify(dhAsinf_sym,[x])

dmKCainf_sym = diff(mKCainf(x),x)
dmKCainf = lambdify(dmKCainf_sym,[x])

# Calcium currents
dmCaLinf_sym = diff(mCaLinf(x),x)
dmCaLinf = lambdify(dmCaLinf_sym,[x])

dmCaTinf_sym = diff(mCaTinf(x),x)
dmCaTinf = lambdify(dmCaTinf_sym,[x])
dhCaTinf_sym = diff(hCaTinf(x),x)
dhCaTinf = lambdify(dhCaTinf_sym,[x])

# Cation current (H-current)
dmHinf_sym = diff(mHinf(x),x)
dmHinf = lambdify(dmHinf_sym,[x])

# Calcium dynamics
CainfL(V)=-α*mCaLinf(V)*(V-VCa)
dCainfL_sym = diff(CainfL(x),x)
dCainfL = lambdify(dCainfL_sym,[x])

CainfT(V)=-β*mCaTinf(V)*hCaTinf(V)*(V-VCa)
dCainfT_sym = diff(CainfT(x),x)
dCainfT = lambdify(dCainfT_sym,[x])

dCainf(V,gCaL,gCaT)=gCaL*dCainfL(V)+gCaT*dCainfT(V)
